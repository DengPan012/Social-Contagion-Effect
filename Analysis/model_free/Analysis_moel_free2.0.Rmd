...
setwd("F:/Lusha Lab/writing up/model_free")
Data=read.csv("F:/Lusha Lab/writing up/model_free/All_Data_model_free.csv")
Data_pre=read.csv("F:/Lusha Lab/writing up/model_free/All_Data_predict.csv")

LogiP=read.csv("F:/Lusha Lab/writing up/model_free/LogiP.csv")
questionaire=read.csv("F:/Lusha Lab/writing up/model_free/question.csv")
```
...
WORD_SIZE = 15
fig11a<-ggplot(data1, aes(x=Risk,y=Rating,fill=Risk))+
  geom_bar(position=position_dodge(), color="white",stat="identity",width=0.9,size=1)+
  ...
  labs( y="Risk-preference Rating", x="Observee's Risk-preference")+
  theme_bw()+
  guides(fill=F)+
  theme
...
fig11b=ggplot(data1, aes(x=Risk,y=Similar,fill=Risk))+
  geom_bar(position=position_dodge(), color="white",stat="identity",width=0.9,size=1)+
  geom_errorbar(aes(ymin=Similar-se,ymax=Similar+se),width=0.2, size=1,position=position_dodge(0.9))+
  scale_fill_manual(values=c("#22DFA9", "#DF22A9"))+
  geom_signif(y_position=c(6), xmin=c(1), xmax=c(2),annotation=c("**"), tip_length=0.05, size=0.4, textsize = 7,vjust = 0.5)+
  labs( y="Similarity Rating", x="Observee's Risk-preference")+
  theme_bw()+
  guides(fill=F)+
  theme
...
```
```{r}
fig11=plot_grid(fig11a, NULL,fig11b, nrow = 1, rel_widths = c(1, 0.2, 1))

jpeg(file = "Fig11_Rating_similarity.jpg",width =3200,height = 1600,units = "px",res =300) #结果保存
fig11
dev.off()
fig11
```
...
fig12b=ggplot(NewData2, aes(x=similar, y=distance)) + 
  geom_point(size=5,alpha=0.5,color="green")+
  geom_smooth(method=lm,se=F,size=1.3,alpha=1,color="black")+
  labs(y="Risk-preference Distance", x="Similarity Rating")+
  theme_bw()+
  theme
...
```
```{r}
fig12=plot_grid(fig12a, NULL,fig12b, nrow = 1, rel_widths = c(1, 0.2, 1))

jpeg(file = "Fig12_Rating_similarity.jpg",width =3200,height = 1600,units = "px",res =300) #结果保存
fig12
dev.off()
fig12
```
...
jpeg(file = "Fig_other_correlationPre_CO.jpg",width =1600,height = 1600,units = "px",res =300) #结果保存
corrplot(b,method="color",type="upper",addCoef.col="gray")
dev.off()
corrplot(b,method="color",type="upper",addCoef.col="gray")
...
fig13a=ggplot(D_con, aes(x= Sequence,y=Correct,fill=factor(Congruence)))  +
...
Congruence")+theme(legend.position = c(0.8, 0.2))+
  theme+xlim(c(-0.5,2.5))


fig13b=ggplot(D_risk, aes(x= Sequence,y=Correct,fill=factor(IsSeeking)))  +
  ...
  geom_signif(y_position=c(0.4), xmin=c(0), xmax=c(0),annotation=c("***"), tip_length=0, size=0, textsize = 7,vjust = 0,color="black")+
  theme_bw()+ 
  labs(x="Session", y="P(correct prediction)",fill="Observee's 
Risk-preference")+theme(legend.position = c(0.7, 0.2))+
  theme+xlim(c(-0.5,2.5))

fig13=plot_grid(fig13a, NULL,fig13b, nrow = 1, rel_widths = c(1, 0.2, 1))

jpeg(file = "Fig13_Predict.jpg",width =3200,height = 1600,units = "px",res =300) #结果保存
fig13
dev.off()
fig13
```
